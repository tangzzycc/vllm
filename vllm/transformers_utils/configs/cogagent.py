...
# Adapted from https://huggingface.co/zai-org/cogagent-chat-hf/blob/main/configuration_cogagent.py
...
class EVACLIPVisionConfig(PretrainedConfig, CLIPVisionConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        num_hidden_layers = vision_kwargs.pop("layers", num_hidden_layers)
        ...
        # The dataclass-generated initializer dispatches to
        # PretrainedConfig.__post_init__ because it appears first in the MRO.
        CLIPVisionConfig.__post_init__(self)
...
class CogAgentConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        template_version: Literal["chat"] = "chat",
        ...
        **kwargs,
        ...
        resolved_image_token_id = (
            vocab_size if image_token_id is None else image_token_id
        )

        self.num_hidden_layers = num_hidden_layers
        if template_version != "chat":
            raise ValueError(
                "CogAgent currently supports only template_version='chat', "
                f"but received {template_version!r}."
            )
        ...
        if vision_config is None:
            ...
            vision_config = vision_config.copy()
            vision_hidden_size = vision_config.pop("hidden_size", 1792)
            if "mlp_intermediate_size" in vision_config:
                mlp_intermediate_size = vision_config.pop("mlp_intermediate_size")
                glu_intermediate_size = vision_config.pop(
            ...
            num_vision_layers = vision_config.pop("num_hidden_layers", None)
            if num_vision_layers is None:
                num_vision_layers = vision_config.pop("layers", 63)
            else:
                vision_config.pop("layers", None)

            outer_hidden_size = vision_config.pop("outer_hidden_size", hidden_size)
            num_vision_heads = vision_config.pop("num_heads", None)
            if num_vision_heads is not None and "head_width" not in vision_config:
                vision_config["head_width"] = vision_hidden_size // num_vision_heads
            ...
            vision_config = EVACLIPVisionConfig(
                ...
                glu_intermediate_size=glu_intermediate_size,
        ...
        if cross_vision_config is None:
            ...
            cross_vision_config = cross_vision_config.copy()
            cross_image_size = cross_vision_config.pop("image_size", cross_image_size)
        ...
        self.cross_hidden_size = cross_vision_config.hidden_size
        ...
        super().__init__(
            ...
            cross_attention_hidden_size=self.cross_hidden_size,
        ...
        self.image_token_id = resolved_image_token_id

        # CrossVisionModel drops the class token before cross-attention.
        self.max_source_positions = cross_vision_config.num_positions - 1
